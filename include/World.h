...
{
public:
    // Can be a cactus or a bird
    struct Obstacle
    {
        Obstacle(sf::Texture& texture)
            : m_Sprite(texture), m_AnimationTime(sf::Time::Zero), m_CurrentFrame(0) {}
...
private:
    ...
    const float m_BirdSpeedMultiplier = 1.25f; // Speed for birds relative to scroll speed

    std::vector<Cloud> m_Clouds;
    sf::IntRect m_CloudRect;
    sf::Clock m_CloudSpawnClock;
    sf::Time m_CloudSpawnInterval = sf::seconds(3.f);
    const float m_CloudSpeedMultiplier = 0.15f; // Speed for clouds relative to scroll speed
